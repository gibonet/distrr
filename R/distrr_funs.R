...
#' @param probs a numeric vector with the desired quantile 
#'     levels (default 0.5, the median)
#' @return The weighted quantile (a numeric vector)
#' @references Ferrez, J., Graf, M. (2007). Enquète suisse sur 
#'     la structure des salaires. Programmes R pour l'intervalle 
#'     de confiance de la médiane. 
#'     (Rapport de méthodes No. 338-0045). Neuchâtel: 
...
wq <- function(x, weights, probs = c(0.5)) {
  ...
  for (i in 1:length(probs)) {
    ...
    res[i] <- ifelse(
      test = abs(tmpS[1, i] - probs[i]) < 1e-10, 
      yes = mean(x[tmpO[, i]]), 
      no = x[tmpO[1, i]]
    )
  }
...
#' @param .variables variables to split data frame by, as a  
#'     character vector (\code{c("var1", "var2")}).
#' @param .fun function to apply to each piece 
#'     (default: \code{\link{jointfun_}})
...
#'     combination of all the categorical variables' 
#'     modalities. In addition to all the modalities, 
#'     each variable will also have a "Total" possibility, 
#'     which includes all the others. The data cube will 
#'     contain marginal, conditional and joint empirical 
#'     distributions...
#' @examples
#' data("invented_wages")
#' str(invented_wages)
...
#' @param  .total modality to eliminate (filter out) 
#'     (default: "Totale")
#' @param .variables a character vector with the names 
#'     of the categorical variables
#' @return a subset of the data cube with only the 
#'     combinations of all variables modalities, without the "margins".
#' 
#' @examples 
#' data(invented_wages)
#' str(invented_wages)
